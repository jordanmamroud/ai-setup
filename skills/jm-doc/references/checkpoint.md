...
A future session reads the project docs for context, then uses those four sections to know what's still in flight.

## Bootstrap

`/jm-doc checkpoint` is self-sufficient — it never requires any other doc mode to have run first.
...
- Never write entries to `## Observations` — that section is user-directed and outside checkpoint's scope. The scaffold placeholder is fine to write once; after that, only the user (or the agent at their request) adds content there.
- Scope is strictly `overview/notes.md`. Don't create or edit the root `README.md` or `ARCHITECTURE.md` — those belong to `/jm-doc readme` and `/jm-doc arc`. Legacy `overview/` files from older layouts (how-it-works, codebase-map) are read-only history; leave them alone.
- Under Codex, the fourth section may be titled `## AGENTS.md candidates` in existing files. Treat it as the same section — append to whichever variant exists; don't create a duplicate. When scaffolding fresh under Codex, use `## AGENTS.md candidates`.
...
- Never write to `## Observations` — that section is user-directed and outside checkpoint's scope.
...
- Include both relative and absolute references. Use `overview/` and `overview/notes.md` as the primary paths; include absolute fallbacks for sessions that start outside the project root.
...
- Don't restate context — point to the overview docs.
