...
The Claude-side bridge (`ui-verify`) is the only existing variant. It's installed, exercised via per-run history, and the architecture has stabilized into a single self-contained folder under `~/mylab/ai-setup/skills/myverify-ui/` with two external symlinks for invocation.
...
- **External symlinks** (verified pointing at the canonical files):
  ...
  - `~/.claude/commands/my-uiverify.md` → `~/mylab/ai-setup/skills/myverify-ui/my-uiverify-command.md`
