---
name: mydoc-spec
description: Create a versioned spec file (spec-v0.md or spec-v1.md) at the project root via a guided interview. State-driven — with no spec, runs a V0 interview from your brain dump; with V0 present, runs a V1 interview centered on the V0→V1 diff. Conversational A/B/C questions, one at a time, each with tradeoffs and a recommendation. Self-contained — works in any project.
...
**Triggers:** the user runs `/mydoc-spec` (optionally with a brain-dump argument: `/mydoc-spec <text>`).
...
If the user invoked `/mydoc-spec <text>`, that text is the brain dump. Skip to Phase 1.

If the user invoked `/mydoc-spec` with no argument, ask:
...
If the user invoked `/mydoc-spec <text>`, that text is the V1 brain dump. Skip to Phase 2.
...
- **Self-contained.** This SKILL.md is the only file the skill needs.
