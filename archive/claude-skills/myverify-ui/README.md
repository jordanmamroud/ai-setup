...
- The four source files in their canonical locations (all in this repo):
  - `~/mylab/ai-setup/claude-skills/myverify-ui/system-prompt.md` — the verifier's system prompt. Read by the bash bridge and inlined into a fresh `claude -p` subprocess. Deliberately NOT in `~/.claude/agents/` so Claude Code does not auto-discover it as a subagent.
  - `~/mylab/ai-setup/claude-skills/myverify-ui/ui-verify` — the bash bridge (the script itself). Symlinked into `~/bin/` so the command is on PATH.
  - `~/mylab/ai-setup/claude-skills/myverify-ui/mcp.json` — declares Playwright MCP for the verifier subprocess only. Wired in via `claude -p --mcp-config "$SCRIPT_DIR/mcp.json"`. Means Playwright MCP is **not** loaded into your other projects' main-agent contexts, only into ui-verify runs.
...
```
~/mylab/ai-setup/claude-skills/myverify-ui/
...
~/bin/
└── ui-verify  →  ~/mylab/ai-setup/claude-skills/myverify-ui/ui-verify          ← puts the bash command on PATH

~/.claude/commands/
└── my-uiverify.md  →  ~/mylab/ai-setup/claude-skills/myverify-ui/my-uiverify-command.md  ← exposes /my-uiverify in any Claude Code session
```

To inspect or edit anything: open the file in `~/mylab/ai-setup/claude-skills/myverify-ui/`. The two symlinks elsewhere are "views" — touching either through its symlink modifies the real file here.
...
(Run from inside `~/mylab/ai-setup/claude-skills/myverify-ui/`. `-f` overwrites any stale symlink.)
...
(at `~/mylab/ai-setup/claude-skills/myverify-ui/system-prompt.md`) inlined via
...
`~/mylab/ai-setup/claude-skills/myverify-ui/system-prompt.md` is the verifier's standing
...
| Bash bridge (canonical) | `~/mylab/ai-setup/claude-skills/myverify-ui/ui-verify` | The actual executable. Spawns `claude -p` with the verifier prompt + tool allowlist + model baked in as flags. |
