---
name: mydoc-overview
description: Create or sync the project's `overview/` folder — one file per section (summary, how-it-works, codebase-map, notes). Scaffolds the folder from template if missing; re-derives the code-derived files in place while leaving human-owned and append-only files untouched.
---
...
Scope is strictly the `overview/` folder. Project README, CLAUDE.md, spec.md, etc. are out of scope.
...
| `overview/notes.md` | Append-only | Mixed ownership. `## Observations` is user-directed (agent writes only when the user explicitly asks). The four checkpoint sections (`## Ideas`, `## Open threads`, `## Resolved`, `## CLAUDE.md candidates`) are owned by `/mycheckpoint`; `/mydoc-overview` scaffolds them empty and never writes there again. One allowed in-place edit: marking/moving a resolved entry. |
...
```markdown
## Summary
`mydoc-overview` is a Claude Code skill that maintains a standardized `overview/` folder for any project. It scaffolds the folder from a template on first run and re-syncs the code-derived files on subsequent runs, leaving the human-owned Summary and the append-only `notes.md` alone. Triggered with `/mydoc-overview`.
```
...
1. Scans the current conversation for new ideas / open threads / threads now resolved / CLAUDE.md candidates.
...
Rules:
- File starts with `# Codebase Map` followed by the `*Last refreshed: YYYY-MM-DD*` line.
- Do not re-list files that already appear inline in `how-it-works.md`. Focus on files and folders the main flow doesn't mention but a reader needs to navigate (configs, fixtures, tests, examples, build artifacts).
...
2. `## Ideas` — **owned by `/mycheckpoint`.** Bulleted entries.
3. `## Open threads` — **owned by `/mycheckpoint`.** Bulleted entries.
4. `## Resolved` — **owned by `/mycheckpoint`.** Bulleted entries, arrived here by being moved out of `## Open threads`.
5. `## CLAUDE.md candidates` — **owned by `/mycheckpoint`.** Bulleted entries.
...
### Rules for the four checkpoint sections (owned by `/mycheckpoint`)
...
- `notes.md` — append-only log. `## Observations` is user-directed (issues, ideas, notes, questions). The four checkpoint sections (Ideas, Open threads, Resolved, CLAUDE.md candidates) are managed by `/mycheckpoint`.
```
