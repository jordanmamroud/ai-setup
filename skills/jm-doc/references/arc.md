...
This replaces the retired `mydoc-overview` pattern of separate `overview/how-it-works.md` and `overview/codebase-map.md` files. Do not create or update `overview/` files.
...
Scope is strictly root `ARCHITECTURE.md`. Do not edit `README.md`, `MEMORY.md`, `SPEC.md`, `AGENTS.md`, or files under `overview/` — the README belongs to `/jm-doc readme`, session notes to `/jm-doc checkpoint`.
...
```markdown
*Placeholder - re-run `/jm-doc arc` once the codebase is established to populate this from real code.*
```
