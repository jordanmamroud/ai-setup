---
name: mydoc-readme
description: Explicit-only skill for creating or refreshing a project's root README.md. Use only when the user explicitly invokes $mydoc-readme.
---

# mydoc-readme

This skill is explicit-only. Do not use it unless the user invokes `$mydoc-readme` or selects it from `/skills`.
...
For README pattern research only, use `references/readme-patterns.md` and `references/repo-readmes-index.md`. Do not load those example READMEs during normal `$mydoc-readme` runs unless the user asks to study README examples or update the default README guidance.
