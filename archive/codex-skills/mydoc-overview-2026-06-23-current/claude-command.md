...
| `overview/notes.md` | Append-only | Mixed ownership. `## Observations` is user-directed (agent writes only when the user explicitly asks). The four checkpoint sections (`## Ideas`, `## Open threads`, `## Resolved`, `## CLAUDE.md candidates`) are owned by `/mydoc-checkpoint`; `/mydoc-overview` scaffolds them empty and never writes there again. One allowed in-place edit: marking/moving a resolved entry. |
...
**Triggers:** use `/mydoc-checkpoint` command, or `!mydoc-checkpoint` from inside Claude Code.
...
5. `## CLAUDE.md candidates` — **owned by `/mydoc-checkpoint`.** Bulleted entries.
