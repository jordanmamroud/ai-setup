...
4. Emits a summary line (absolute path + one-line of what changed) + a copy-pasteable fresh-session prompt pointing at `overview/notes.md` only, ending with a one-line "Next:" action and the fixed-phrasing constraint capping the first response at ~200 words with A/B/C/D options.
